...
def get_consts_values():
    ...
    message.append("IS_BUCKET_KEY = %s" % (f"{IS_BUCKET_KEY[:3]}********{IS_BUCKET_KEY[-1:]}" if IS_BUCKET_KEY else ""))
    message.append("IS_ACCESS_KEY = %s" % (f"{IS_ACCESS_KEY[:3]}******************************{IS_ACCESS_KEY[-3:]}" if IS_ACCESS_KEY else ""))
...
def stream_to_logstash(humidity, temperature, ls_hosts):
    ...
    for host, port in ls_hosts:
        try:
            ...
            print(f"Couldn't connect to Logstash host ({host}:{port}): {error_msg}")
    else:
        print(f"Couldn't successfully connect to any Logstash host ({LOGSTASH_HOSTS})")
...
def main():
    ...
    while True:
        ...
        if not math.isclose(temperature, previous_temperature, abs_tol=LOG_IF_CHANGED_BY) \
            ...
            print(f"{now} {temperature:.2f}{'F' if CONVERT_TEMP_TO_F else 'C'} {humidity:.2f}%%")
            if CSV_OUT_FILE_NAME:
                with open(CSV_OUT_FILE_NAME, "a", encoding="utf-8") as csv_file:
